...
stringOfHistory = '/data/esm1g14/SecondHalf/CascadeLearning/VGG_Results/Run6_gradients/'
# stringOfHistory = './VGG_Results/Run1_gradients/'
...
if not os.path.isfile(stringOfHistory+'history'+str(depth)+'.txt'):
  ...
  model = get_vgg_style_net(dropout=False,outNeurons=512,nb_classes=nb_classes,blocks=[3,3]) #GET THE MODEL
...
model = get_vgg_style_net(dropout=True,outNeurons=512,nb_classes=nb_classes,blocks=[3,3]) #GET THE MODEL
...
learningCall = LearningRateC(X_val,Y_val,X_test,Y_test,datagen,batch_size,patience=75,windowSize=20,gradient=True,plot_folder=stringOfHistory)
#TRAIN THE END-END MODEL
print('TRAINING END-END MODEL WITH DEPTH ' + str(depth))
tmpHistory = model.fit_generator(datagen.flow(X_train, Y_train, batch_size=batch_size),steps_per_epoch=np.ceil(1.*len(X_train)/batch_size).astype(int),epochs=200,verbose=2,callbacks=[learningCall])
...
if saveResults:
  save_full_model(model,None,stringOfHistory,'end_end_'+str(depth)+'2.h5')
  save_full_model(None,history,stringOfHistory,name=str(depth)+'2all')
